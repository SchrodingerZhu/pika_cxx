...
pika::graph::TableEntry::TableEntry(const pika::clause::Clause *instance, size_t topological_order)
        : instance(instance), candidates({}), topological_order(topological_order) {}

pika::graph::ClauseTable::ClauseTable(std::vector<const pika::clause::Clause *> specials,
                                      std::vector<const pika::clause::Clause *> terminals,
                                      std::string_view target, const clause::Clause *toplevel)
        : specials(std::move(specials)), terminals(std::move(terminals)), memo_table(target),
          current_pos(target.size() + 1), toplevel(toplevel) {

}

char pika::graph::ClauseTable::get_current() const {
    return (current_pos > memo_table.target.size()) ? EOF : memo_table.target[current_pos - 1];
}
...
#ifdef PIKA_DEBUG
        std::cout << "added: "
                  << abi::__cxa_demangle(typeid(*i).name(), nullptr, nullptr, nullptr)
                  << ", at: "
                  << this->current_pos - 1
                  << std::endl;
#endif
        this->column.template emplace(this->template at(typeid(*i)).topological_order, i);
    }
}
...
void pika::graph::ClauseTable::try_add(const pika::clause::Clause *tag, size_t length, size_t fst_idx,
                                       std::vector<std::shared_ptr<memotable::Match>> subs) {
    auto key = memotable::MemoKey{tag, current_pos - 1};
#ifdef PIKA_DEBUG
    std::cout << "matched " << abi::__cxa_demangle(typeid(*tag).name(), nullptr, nullptr, nullptr)
              << ", at " << this->current_pos - 1
              << ", content: " << this->memo_table.target.substr(this->current_pos - 1, this->current_pos - 1 + length)
              << ", subs_len: " << subs.size()
              << ", length: " << length;
#endif
...
bool pika::graph::ClauseTable::match_column() {
    ...
    while (!column.empty()) {
        ...
        std::cout << "trying to parse: "
                  << abi::__cxa_demangle(typeid(*top).name(), nullptr, nullptr, nullptr)
                  << ", at: "
                  << this->current_pos - 1
                  << std::endl;
#endif
...
pika::graph::ClauseTable pika::graph::construct_table(const pika::clause::Clause &toplevel, std::string_view target) {
    std::vector<const pika::clause::Clause *> terminals;
    ...
    /*
     * The following things may always success, we need to check them everytime the priority queue is empty
     * They are behaving like a self-looping.
     */
    for (auto i : nodes) {
    ...
    ClauseTable table(std::move(specials), terminals, target, toplevel.get_instance());
