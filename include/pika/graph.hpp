...
namespace pika {
    namespace graph {
        struct TableEntry {
            ...
            TableEntry(const pika::clause::Clause *instance, size_t topological_order);
        };

        using ParsingItem = std::pair<size_t, const pika::clause::Clause *>;

        struct ClauseTable : public absl::flat_hash_map<std::type_index, TableEntry> {
            ...
            std::priority_queue<ParsingItem, std::vector<ParsingItem>, std::greater<>> column{};
            size_t current_pos;
            const clause::Clause* toplevel;

            explicit ClauseTable(std::vector<const pika::clause::Clause *> specials,
                                 std::vector<const pika::clause::Clause *> terminals,
                                 std::string_view target, const clause::Clause* toplevel);

            char get_current() const;

            void try_add(const clause::Clause *, size_t length, size_t fst_idx,
        ...
        ClauseTable construct_table(const clause::Clause &toplevel, std::string_view target);
    }
}
