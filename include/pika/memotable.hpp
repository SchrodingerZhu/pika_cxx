...
namespace pika {
    namespace memotable {
        struct MemoKey {
            ...
            const pika::clause::Clause  * const tag;

            template<class Clause, typename = std::enable_if_t<std::is_base_of_v<pika::clause::Clause, Clause>>>
            MemoKey(const Clause &tag, size_t start_position) noexcept : tag(&tag), start_position(start_position),
            ...
            [[nodiscard]] bool is_based_on() const noexcept {
                return dynamic_cast<const C *>(tag) != nullptr;
            }
        };
