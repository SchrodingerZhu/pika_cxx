...
PIKA_DECLARE(
    Primary,
    PIKA_ORD(PIKA_SEQ(PIKA_CHAR('('), Additive, PIKA_CHAR(')')), Number),
    true);

PIKA_DECLARE(
    Multiplicative,
    PIKA_ORD(PIKA_SEQ(Primary, PIKA_CHAR('*'), Multiplicative), Primary),
    true);

PIKA_DECLARE(
    Additive,
    PIKA_ORD(
        PIKA_SEQ(Multiplicative, PIKA_CHAR('+'), Additive), Multiplicative),
    true);

PIKA_DECLARE(
    Toplevel,
    PIKA_SEQ(PIKA_FIRST, Additive, PIKA_NOT_FOLLOWED_BY(PIKA_ANY)),
    true);
PIKA_DECLARE(
    Add, PIKA_ORD(PIKA_SEQ(Add, PIKA_CHAR('+'), Number), Number), true);
TEST(Clause, Display)
{
    auto target =
...
TEST(Clause, PackratMatch)
{
    ...
    }
    {
        pika::memotable::MemoTable table(
            "(1+2)*(123*4444*(1+555*2+3))*(3)+23*(5+5)*(123*(5+1234))");
        auto result = Toplevel().packrat_match(table, 0);
        EXPECT_TRUE(result);
    }
    {
